              ...
              {!isPageLoading && !isLoading &&
                <Fragment>
                  <Breadcrumb>
                    ...
                    <BreadcrumbItem><NavLink to={`${subDirectory}/users`}>User</NavLink></BreadcrumbItem>
                    <BreadcrumbItem>{uid}</BreadcrumbItem>
                    <BreadcrumbItem active>{formType}</BreadcrumbItem>
                  </Breadcrumb>
                  <Container fluid>
                    <div className="mb-4 subnav-div">
                      {uid === "new" && <NavLink to={{ pathname: `${subDirectory}/user/register/new`, query: this.props.location.query }} className={classNames('subnav', { disabled: uid !== 'new' })} activeClassName="subnav-active" exact={true} >Registration</NavLink>}
                      {formTypeOrder.map((formType) =>
                        uid !== "new" ?
                          <NavLink key={formType} to={{ pathname: `${subDirectory}/${clientid}/${entityid}/${branch}/${uid}/edit/${formType}`, query: this.props.location.query }} className={classNames('subnav', { disabled: uid === 'new' })} activeClassName="subnav-active" exact={true} >{formTypeKeys[formType]['label']}</NavLink> : <span className="btn btn-link disabled">{formTypeKeys[formType]['label']}</span>
                      )
                      }
                    </div>
