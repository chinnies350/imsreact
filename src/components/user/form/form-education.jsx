...
export default class EducationForm extends Component {
  ...
  schema = {
    ...
    percentage: Joi.number().min(1).max(100).required().label("course  Percentage"),
    ...
    if (userData) {
      ...
      if (education) {
        education.map((education, i) => {
          ...
          var fromYear = education.from.split("/")[1];
          var toMonth = education.to.split("/")[0];
          //* var toYear = 20 + education.to.split("/")[1];
          var toYear = education.to.split("/")[1];
  ...
  addCourse = async (type, percentage, name, affiliate, from, to, otherDetails, projectDetails) => {
    ...
    if (values.educationDetails) educationDetails = values.educationDetails;
  ...
  addProject = async (projectName, projectDate, projectStatus, associatedWith, projectURL, projectdescription) => {
    ...
    const values = this.formApi.getState().values;
    let projectsDetails = [];
    if (values.projectDetails) projectsDetails = values.projectDetails;
      ...
      if (item && item.from && item !== "") {
        ...
        if (beforeInMoment.isBefore(afterInMoment)) {
          ...
          if (res.data.statusCode === 1) {
            ...
            props.history.push(`${subDirectory}/${data.client}/${data.entity}/${data.branch}/${data.uid}/edit/organization`);
          } else if (res.data.statusCode === 0) ToastService.Toast(res.data.message, "default");
          else ToastService.Toast(`Failed to update Education Details`, "default");
        } else {
          ToastService.Toast(`Check the Month Values `, "default");
        }
      } else {
        ToastService.Toast(`Please Select Proper Month values`, "default");
      }
    }
  };
              ...
              <div>
                <section>
                  ...
                  <Row className="justify-content-end">
                    <button className="btn btn-link btn-sm" type="button" onClick={() => this.addCourse("", "", "", "", "", "", "")}>
                      + Add Course
                    </button>
                  </Row>
                  {formState.values.educationDetails &&
                    formState.values.educationDetails.map((education, i) => (
                      <Scope scope={`educationDetails[${i}]`} key={i}>
                        <Row>
                          ...
                          <Col sm={12} md={4}>
                            <Input field="percentage" label="Percentage*" validateOnBlur validate={(e) => this.validateProperty("percentage", e)} />
                          </Col>
                          <Col sm={12} md={4}>
                            <Input field="name" label="Course Name*" validateOnBlur validate={(e) => this.validateProperty("name", e)} />
                          </Col>
                          <Col sm={12} md={4}>
                            <Input field="affiliate" label="Affiliated To*" validateOnBlur validate={(e) => this.validateProperty("affiliate", e)} />
                          </Col>
                          <Col sm={12} md={4}>
                            {console.log({ fromYear, toYear, fromMonth, toMonth })}
                            {console.log(parseInt(education.from.split("/")[1]), parseInt(education.from.split("/")[0]), parseInt(education.to.split("/")[1]), parseInt(education.to.split("/")[0]))}
                            <MP
                              field="from"
                              label="From*"
                              name="from"
                              // initialValue={education.from}
                              // value={education.from}

                              year={parseInt(education.from.split("/")[1])}
                              month={parseInt(education.from.split("/")[0])}
                            ></MP>
                          </Col>
                          <Col sm={12} md={4}>
                            <MP
                              ...
                              label="To*"
                              name="to"
                              //  initialValue={education.to}
                              // value={education.to}
                              // onChange={()=>this.setState({to:education.to},()=>{console.log(education.to,"hihihihi")})}
                              year={parseInt(education.from.split("/")[1]) <= parseInt(education.to.split("/")[1]) ? parseInt(education.to.split("/")[1]) : ""}
                              month={parseInt(education.to.split("/")[0]) || 0}
                            ></MP>
                          </Col>
                          <Col sm={12} md={12}>
                            <Textarea field="otherDetails" label="Description" validateOnBlur validate={(e) => this.validateProperty("otherDetails", e)} />
                          </Col>
                          {formState.values.educationDetails.length > 1 && i !== 0 ? (
                            <Col sm={12}>
                              <button onClick={() => this.removeCourse(i)} className="btn btn-link btn-sm">
                                Remove
                              </button>
                            </Col>
                          ) : (
                    ...
                    <Row>
                      <Col sm={12}>
                        <div style={{ fontSize: "10pt" }}>
                          <span style={{ color: "#e91e63", cursor: "pointer" }} onClick={this.toggle}>
                            Click Here
                          </span>{" "}
                ...
                {this.state.modal ? (
                  <section>
                    ...
                    <Row className="justify-content-end">
                      <button className="btn btn-link btn-sm" type="button" onClick={() => this.addProject("", "", "", "", "", "")}>
                        + Add Project
                      </button>
                    </Row>
                    {formState.values.projectDetails &&
                      formState.values.projectDetails.map((projectDetails, i) => (
                        <Scope scope={`projectDetails[${i}]`} key={i}>
                          <Row>
                            <Col sm={4}>
                              <Input field="projectName" label="Project Name*" validateOnBlur validate={(e) => this.validateProperty("projectName", e)} />{" "}
                            </Col>
                            <Col sm={12} md={4}>
                          ...
                          <Row>
                            ...
                            <Col sm={4}>
                              <Input field="projectURL" label="Project URL*" validateOnBlur validate={(e) => this.validateProperty("projectURL", e)} />
                            </Col>
                          </Row>
                          <Row>
                            <Col sm={12}>
                              <Textarea field="projectdescription" label="Description" validateOnBlur validate={(e) => this.validateProperty("projectdescription", e)} />
                            </Col>
                          </Row>
                          {i !== 0 ? (
                            <Col sm={12}>
                              <button onClick={() => this.removeProject(i)} className="btn btn-link btn-sm">
                                Remove
                              </button>
                            </Col>
                          ) : (
                ...
                <div className="text-right">
                  ...
                  {formState.values.educationDetails &&
                    formState.values.educationDetails.every((education) => {
                      return (
                        !Object.values(_.omit(education, ["otherDetails"])).some((value) => {
