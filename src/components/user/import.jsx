                ...
                {!isPageLoading && <React.Fragment>
                    ...
                    <div className="d-md-flex align-items-md-center justify-content-md-between">
                        ...
                        <div>
                            ...
                            <NavLink className="btn btn-primary btn-sm" to={`${subDirectory}/${type}/new`}>+ Add {labels[type][1]}</NavLink>
                        </div>
                    </div>
...
function clientLinkFormatter(cell, row, rowIndex, formatExtraData) {
    ...
    switch (type) {
        case "entity":
            links.push(<NavLink to={`${subDirectory}/${prefixUrl}/entities`} className='badge badge-light'>{cell}</NavLink>)
            break;
        case "branch":
            links.push(<NavLink to={`${subDirectory}/${prefixUrl}/branches`} className='badge badge-light'>{cell}</NavLink>)
            break;
        case "department":
            links.push(<NavLink to={`${subDirectory}/${prefixUrl}/departments`} className='badge badge-light'>{cell}</NavLink>)
            break;
        case "batch":
            links.push(<NavLink to={`${subDirectory}/${prefixUrl}/batches`} className='badge badge-light'>{cell}</NavLink>)
            break;
        default:
            links.push(cell)
...
function actionsFormatter(cell, row, rowIndex, formatExtraData) {
    ...
    links.push(<NavLink to={`${subDirectory}/${row.type}/${row.id}`} className='badge badge-success'>View</NavLink>)
    links.push(<NavLink to={`${subDirectory}/${row.type}/${row.id}/edit`} className='badge badge-warning'>Edit</NavLink>)
    links.push(<NavLink to={`${subDirectory}/${row.type}/${row.id}/delete`} className='badge badge-danger'>Delete</NavLink>)
