...
import { staffAttendanceAdd, staffEditAttendance, getStaffAllreports } from "services/attendanceService";
...
export default class AddAttendance extends Component {
  ...
  async getSubjects() {
    var subjectsArray = [];
    const {
      data: { client, batch, entity, department, branch },
    } = this.state;
  ...
  async homeworkReport(data = []) {
    ...
    await _.map(data).forEach(async (item) => {
      ...
      temp.push({ staffId: uid, remark: remarks, status: "P" });
    });
    ...
    if (attendanceaction === "edit") {
      ...
      if (data.length !== 0) {
        ...
        const attendanceStatus = await staffAttendanceAdd(obj);
  ...
  async updateAttendance(data) {
    ...
    const updateAttDetails = await staffEditAttendance(edittemp);
  ...
  async getStudentList(data) {
    ...
    const res = await getStaffAllreports(params);
    console.log(res);
    if (res.data.statusCode === 1) {
      var test = res.data.data;
      for (var i = 0; i < test.length; i++) {
        studentList.push({ name: test[i].name, uid: test[i].staffId, remarks: "", mark: "" });
      }
  ...
  async payloadData(data) {
    ...
    await _.map(data).forEach(async (item) => {
      ...
      if (Status && UserId) {
        ...
        tempdata.push({ staffId: postUser, remark: "", status: postStatus });
      }
    });
    ...
    console.log(temp);
    const attendanceStatus = await staffAttendanceAdd(temp);
    if (attendanceStatus.data.statusCode === 1) {
      ToastService.Toast(attendanceStatus.data.message, "default");
      this.props.props.history.push(`${subDirectory}/attendance/attendance`);
    }
  }
              ...
              <section>
                ...
                <Row>
                  ...
                  <Col sm={6} md={3}>
                    ...
                    <CustomDatePicker field="date" isOutsideRange={isOutsideRange} id="date" date={moment(date)} validate={(e) => this.validateProperty("date", e)} onChange={this.dateValue} onBlur={(e) => this.validateProperty("date", e)} numberOfMonths={1} />
                  </Col>
                  <Col sm={6} md={3}>
                  ...
                  <Col sm={6} md={3}>
                    <Input field="period" label="Period*" name="period" validate={(e) => this.validateProperty("period", e)} />
                  </Col>
                </Row>
              </section>
          ...
          <section>
            <Form>
              <Row>
                ...
                <Col sm={12} md={6} style={{ textAlign: "right" }}>
                  <button className="btn btn-outline-secondary btn-sm" onClick={() => this.bulkModalToggle()}>
                    Bulk Upload
                  </button>{" "}
                  &nbsp;
                </Col>
            ...
            <Row>
              <Col md={12} className="d-flex justify-content-center">
                <button type="submit" onClick={() => this.onSubmit()} className="btn btn-primary btn-sm">
                  Submit
                </button>
              </Col>
            </Row>
                  ...
                  <section>
                    ...
                    <Row>
                      ...
                      <Row className="justify-content-end">
                        <button type="button" className="btn btn-warning cancel" onClick={() => this.bulkModalToggle()} style={{ marginRight: "20px" }}>
                          Cancel
                        </button>
                        <button type="submit" className="btn btn-primary btn-sm" onSubmit={(e) => this.saveDetails(e)}>
                          Save
                        </button>
                      </Row>
                    </Row>
