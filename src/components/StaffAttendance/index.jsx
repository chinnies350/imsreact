...
import { getStaffAttendancelist, getHoliday } from 'services/attendanceService';
...
console.log("staffattendance")
  ...
  rightsData = async (session) => {
    ...
    var moduleName = "staffs"
    if (attendanceType === "holiday") moduleName = "institutionmasters"
  ...
  attendanceList = async () => {
    ...
    const attendancestatus = await getStaffAttendancelist(params);
      ...
      _.map(temp, item => {
        _.map(_.keys(item), v => {
          if (typeof item[v] === 'object') {
            let obj = { name: item["name"], studentId: item["staffId"] }
        ...
        {!isPageLoading && !isLoading &&
          <Fragment>
            ...
            <Breadcrumb>
              ...
              <BreadcrumbItem><NavLink to={`${subDirectory}/staffattendance/attendance`}>Staff Attendance</NavLink></BreadcrumbItem>
                          ...
                          attendanceType !== 'holiday' ?
                            <Row>
                              ...
                              <Col sm={6} md={3}>
                                ...
                                <CustomDatePicker field="date" isOutsideRange={isOutsideRange} id="date" date={moment(date)} validate={e => this.validateProperty('date', e)} onChange={this.dateValue} onBlur={(e) => this.validateProperty('date', e)} numberOfMonths={1}/>                              </Col>
                              <Col sm={6} md={3}>
