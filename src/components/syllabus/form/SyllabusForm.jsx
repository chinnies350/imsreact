...
export default class Syllabus extends Component {
    ...
    formStateCheck = async (data1,data) => {
        console.log(data1,data)
        data1.subjectcode = data1.syllabusDatas.refId
      
        let temp = [];
        await temp.push(data1["syllabusDatas"]);
        await delete data["syllabusDatas"]
        data1["children"] = temp
        data1["lesson"] = temp
        await this.setState({ data });
        try {
            ...
            await this.clientDatas('department');
            await this.clientDatas('batch');
            await this.formApi.setValues(data1);
        } catch (err) {
        }
    }
    ...
    resetForm = () => { //Clear the form values afer the submission              
        this.formApi.reset()
        let path = `${subDirectory}/course/syllabus-list` //Redirect the page after updated the datas
    ...
    onSubmit = async () => { //Store Datas to the APIs
        ...
        console.log('syllabus submit form')
        ...
        console.log(data,data1,this.state)
        if (actionTypes === 'add') {
            let addparams = `client=${data.client}&entity=${data.entity}&branch=${data.branch}`
            let addDatas = { "refId": data1.subjectcode, "lesson": data1.children }
            response = await addSyllabus(addparams, addDatas)
        console.log(response)
        } else if (actionTypes === 'edit') {
            // let updateDatas = { "lesson": data.children }
            let updateparams = `client=${data.client}&entity=${data.entity}&branch=${data.branch}&refId=${data1.lesson[0].refId}&id=${data1.lesson[0]._id}`
            let updateDatas = { "lesson": data1.children }
            // let updateparams = `client=${client}&entity=${entity}&branch=${branch}&refId=${data.lesson[0].refId}&id=${data.lesson[0]._id}`
            console.log(updateparams,updateDatas)
    ...
    render() {
        const {data,client, entity,branch,clientIds, entityIds, branchIds, isClient, isEntity, isBranch } = this.state
        console.log(this.state)
        return (
            <Fragment>
                <Form getApi={this.setFormApi} onSubmit={this.onSubmit} >
                  ...
                  {/* Client as Group  and Entity as Institution*/}
                                            ...
                                            <CustomSelect field="client" label="Group*" name="client" getOptionValue={option => option.code} getOptionLabel={option => option.name} options={clientIds}
                                                validateOnBlur validate={e => this.validateProperty('client', e)} onChange={this.handleChange} />
                                        </Col>
                                    }
                                    {isEntity &&
                                        <Col sm={12} md={3}>
                                            <CustomSelect field="entity" label="Institution*" name="entity" getOptionValue={option => option.code} getOptionLabel={option => option.name} validateOnBlur validate={e => this.validateProperty('entity', e)} options={entityIds}
