...
import EventForm from 'components/schedule/form/EventSchedule';
...
import ExamForm from 'components/schedule/form/ExamSchedule';
import TermForm from 'components/schedule/form/TermSchedule';
import TimeTableForm from 'components/schedule/form/TimeTableSchedule';
import AttendanceForm from 'components/schedule/form/AttendanceSchedule';
...
export default class UserForm extends Component {
  ...
  render() {
    ...
    const { keys: formTypeKeys, order: formTypeOrder } = Static.scheduleFormTypes();
                ...
                <Container fluid>
                  <div className="mb-4">
                     ...
                     {formTypeOrder.map((scheduleform) =>
                      <NavLink key={scheduleform} to={{ pathname: `${subDirectory}/schedule/${actiontype}/${scheduleform}`, query: this.props.location.query }} className={classNames('btn btn-link', { disabled: actiontype === 'edit' })} activeClassName="btn-primary" exact={true} >{formTypeKeys[scheduleform]['label']}</NavLink>
                    )
                    }
                  </div>
