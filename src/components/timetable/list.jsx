...
export default class Timetable extends Component {
  ...
  actionsFormatter = (cell, row, rowIndex, formatExtraData) => {
    let links = []
    links.push(<div onClick={() => this.editFun(`${subDirectory}/staff/edit/staffallocation/`, row)} className='badge badge-warning'>Edit</div>)
  ...
  deleteFun = async (row) => {
    console.log('delete timetable work allocation')
    let params;
    const {  _id } = row
    if (row.batch) {
      params = `client=${row.client}&entity=${row.entity}&branch=${row.branch}&department=${row.department}&batch=${row.batch}&staffId=${row.staffId}&id=${row._id}`
    } else {
      params = `client=${row.client}&entity=${row.entity}&branch=${row.branch}&department=${row.department}&staffId=${row.staffId}&id=${row._id}`
    }
  ...
  ToastService.Toast(res.data.message, 'default')
    ...
    this.resetForm()
    setTimeout(`location.href='${subDirectory}/timetable/WorkAllocation'`);
