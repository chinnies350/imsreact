...
export default class AddStaffAllocation extends Component {
  ...
  timeValue = async (time, field) => { // Get time from the timepicker
    ...
    handleSubmit = async () => {
      console.log('staff allocation edit')
    const { actiontype } = this.props
  ...
  resetForm = () => {
    // this.formApi.reset()
    let path = `${subDirectory}/timetable/WorkAllocation` //Redirect the page after updated the datas
    this.props.props.history.push({
      pathname: path,
    })
  }
          ...
          {({ formApi, formState }) => (
            <div>
              ...
              <section>
                ...
                <Row>
                  <Col sm={6} md={3}>
                    <CustomSelect field="batch" label="Batch*" name="batch"
                     getOptionValue={option => option.code}
                      getOptionLabel={option => option.name}
                       options={batchIds} 
                       validateOnBlur 
                       validate={e => this.validateProperty('batch', e)} onChange={this.handleChange} />
                  </Col>

                  <Col sm={6} md={3}>
                    <label> Date*</label>
                    <CustomDatePicker field="date" isOutsideRange={isOutsideRange}
                     id="date" date={dob}
                     validateOnBlur 
                  ...
                  <Col sm={6} md={3}>
                    <CustomSelect field="staffId" 
                    label="Staff*" name="staffId" 
                    getOptionValue={option => option.uid} 
                    getOptionLabel={option => option.name} 
                    options={staffs} validateOnBlur 
                    validate={e => this.validateProperty('staffId', e)}
                    onChange={this.handleChange} />

                  </Col>
                  <Col sm={6} md={3}>
                    <CustomSelect field="staffName" label="Staff Name*" 
                    name="staffName" getOptionValue={option => option.name}
