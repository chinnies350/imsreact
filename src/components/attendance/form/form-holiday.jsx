...
export default class HolidayForm extends Component {
  ...
  async componentDidMount() {
    ...
    console.log(data,this.props,"holiday form")
...
if(actiontype === "add"){
  const currentDate= { from:new Date(), to:new Date() } 

  this.dateValue(currentDate, "date")
}
    if (actiontype === "edit") {
  ...
  handleError(...err) {
    return ToastService.Toast("Something went wrong.Please lltry again later", 'default');
  }
            ...
            <div>
              ...
              {isBatch && <section>
                ...
                <Row>
                  {isClient && <Col sm={12} md={3}>
                      {/* Client as Group */}
                    <CustomSelect field="client" label="Group*" name="client" getOptionValue={option => option.code} validateOnBlur validate={e => this.validateProperty('client', e)} getOptionLabel={option => option.name} options={clientIds}
                      onChange={this.handleChange} />
                  </Col>}
                  {isEntity && <Col sm={12} md={3}>
                      {/* Entity as Institution*/}
                    <CustomSelect field="entity" label="Institution*" name="entity" getOptionValue={option => option.code} validateOnBlur validate={e => this.validateProperty('entity', e)} getOptionLabel={option => option.name} options={entityIds}
                      onChange={this.handleChange} />
                  </Col>}
              ...
              <section>
                ...
                <Row>
                  ...
                  <Col sm={12} md={5}>
                    <label>Date*</label>
                    <DRP1 field="date" label="Date" id="date" startDate={moment(formState.values.startDate)} endDate={moment(formState.values.endDate)} onChange={(data) => this.dateValue(data, "date") } validateOnBlur validate={e => this.validateProperty('date', e)} />
                  </Col>
                </Row>
