...
export default class MarkList extends Component {
  ...
  initTableData = async () => {
    console.log("fgbggggdhg ccccccccccccccc")
  ...
  adduserNavigation() {
    ...
    switch (form) {
      case "gpa":
        return <NavLink className="btn btn-primary btn-sm" to={`${subDirectory}/mark/add/gpa`}>+ Add GPA Mark</NavLink>;
      case "cce":
        return <NavLink className="btn btn-primary btn-sm" to={`${subDirectory}/mark/add/cce`}>+ Add CCE Mark</NavLink>;
      default:
        return <NavLink className="btn btn-primary btn-sm" to={`${subDirectory}/mark/add/gpa`}>+ Add GPA Mark</NavLink>;
  ...
  getColumnHeaders(type, prefixUrl = "", dynamicLabels = {}) {
    ...
    let def = {
      ...
      "Client": {
        dataField: 'client', text: `Group`, filter: this.getTextFilter(), sort: true
      },
  ...
  actionFormater = (cell, row, rowIndex, formatExtraData) => {
    let links = []
    links.push(<div onClick={() => this.scholasticMarkEdit(`${subDirectory}/mark/view/gpa`, row)} className='badge badge-success'>View</div>)
    // links.push(<NavLink to={`/user/${row.uid}/edit/login`} className='badge badge-warning'>Edit</NavLink>)
    links.push(<div onClick={() => this.scholasticMarkEdit(`${subDirectory}/mark/edit/gpa`, row)} className='badge badge-warning'>Edit</div>)
  ...
  coActionFormater = (cell, row, rowIndex, formatExtraData) => {
    let links = []
    links.push(<div onClick={() => this.scholasticMarkEdit(`${subDirectory}/mark/view/cce`, row)} className='badge badge-success'>View</div>)
    // links.push(<NavLink to={`/user/${row.uid}/edit/login`} className='badge badge-warning'>Edit</NavLink>)
    links.push(<div onClick={() => this.scholasticMarkEdit(`${subDirectory}/mark/edit/cce`, row)} className='badge badge-warning'>Edit</div>)
    // links.push(<div onClick={() => this.gradeDelete(row)} className='badge badge-danger'>Delete</div>) 
    return <div className="actions">{links.concat(" ")}</div>
  }
