...
export default class CCE extends Component {
  ...
  feildCheck = async () => {
    ...
    const { userType, userLevel, client, entity, branch, department, batch, branchId, code, departmentId, batchId } = sessionData;
    let switchType = "";
  ...
  init = async () => {
    ...
    if (actionType === "edit") this.setState({ isEditForm: true });
    ...
    if (state !== undefined && state.isFromView) return this.getSampleData(state);
  };
  ...
  clientDatas = async (name) => {
    ...
    switch (name) {
      ...
      case "department":
        ...
        break;
      case "type":
        if (type === "co-scholastic") return this.getCCESkills();
  ...
  getStudentList = async () => {
    const {
      data: { client, branch, entity, department, batch, subject, term, examId, type },
      data,
    } = this.state;
      ...
      <Fragment>
        <Container fluid>
          <Breadcrumb>
            ...
            <BreadcrumbItem>
              <NavLink to={`${subDirectory}/mark/cce`}>CCE Mark List</NavLink>
            </BreadcrumbItem>
                    ...
                    <Row>
                      <Col sm={12} md={6}>
                        ...
                        <div style={{ fontSize: "12px" }}>
                          <p>1. Click on the Marks Column to enter the Marks</p>
                          <p>2. Click on the Remarks Column to give the remark for the Student</p>
                        </div>
                      </Col>
                    </Row>
                    <BootstrapTable keyField="studentId" data={studentList} columns={columns} bootstrap4 classes="table table-bordered table-hover table-sm" wrapperClasses="table-responsive" noDataIndication={"No data to display here"} cellEdit={cellEdit} />
                  </div>
                )}
