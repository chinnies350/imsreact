...
import { addMarkReportGpa, updateMarkReport, studentDataList } from 'services/markReportService'
...
export default class GPA extends Component {
  ...
  feildCheck = async () => {
    ...
    console.log(this.state)
    const { userType, userLevel, client, entity,code, branchId,branch, department, batch } = sessionData;
...
console.log("gggggggggg")
  ...
  //     let params = `client=${data.client}&entity=${data.entity}&branch=${data.branch}&type=subject&departmentId=${departmentId}`;
  ...
  //       // var studentList = res.data.response
...
console.log(data)
    try {
      if (!isEditForm) {
        ...
        res = await addMarkReportGpa(data);
  ...
  render() {
    const { clientIds,department,client,entity, entityIds, branchIds, departmentIds, batchIds, formActionType, examNameList, subjectNameList, termList, isTableLoading, data: { studentList },
      ...
      <Fragment>
        <Container fluid>
          <Breadcrumb>
            ...
            <BreadcrumbItem><NavLink to={`${subDirectory}/mark/gpa`}>GPA Mark List</NavLink>
            </BreadcrumbItem>
              ...
              <div>
                {isBatch && <section>
                  <Row>
                    ...
                    {isDepartment &&
                      <Col sm={6} md={3}>
                        <CustomSelect field="department" label="Department*" 
                       
                        getOptionValue={option => option.code} getOptionLabel={option => option.name} validateOnBlur validate={e => this.validateProperty('department', e)} onChange={this.handleChange} options={departmentIds} disabled={formActionType} />
                      </Col>}
