...
import { getMarkListGpa, getMarkList } from "services/markReportService";
...
export default class MarkDataList extends React.Component {
  state = {
    data: { client: "", entity: "", department: "", branch: "", batch: "" },
    tableData: {
      gpa: [],
      cce: {},
    },
  ...
  adduserNavigation() {
    ...
    switch (form) {
      case "gpa":
        return (
          <NavLink className="btn btn-primary btn-sm" to={`${subDirectory}/mark/add/gpa`}>
            + Add GPA Mark
          </NavLink>
        );
      case "cce":
        //!if condition added
        if (switchType == "staff" || switchType === "department" || switchType === "branch" || switchType === "batch") {
          return (
            <NavLink className="btn btn-primary btn-sm" to={`${subDirectory}/mark/add/cce`}>
              + Add CCE Mark
            </NavLink>
          );
        } else {
  ...
  async selectoptGet(url, type) {
    ...
    if (data.data.statusCode === 1) {
      ...
      // await ToastService.Toast(`${type} Data Not Found!!!`, "default")
    }
  }
  ...
  onSubmit = async () => {
    ...
    switch (listType) {
      case "gpa":
        return this.getGPAMarkList("GPA");
      case "cce":
  ...
  getGPAMarkList = async () => {
    const {
      data: { client, entity, branch, department, batch, examId, term, subject },
      tableData,
    } = this.state;
    ...
    try {
      const res = await getMarkListGpa(params);
  ...
  getCCEMarkList = async (type) => {
    const {
      data: { client, entity, branch, department, batch, term, examId, subject },
      tableData,
    } = this.state;
    let params = `client=${client}&entity=${entity}&branch=${branch}&department=${department}&batch=${batch}&term=${term}&examId=${examId}&subject=${subject}&type=${type}`;
    try {
  ...
  render() {
    const { listType } = this.props.match.params;
    const { isPageLoading, isLoading, clientIds, entityIds, branchIds, departmentIds, batchIds, examNameList, subjectNameList, termList, tableData, isTableLoading, isClient, isEntity, isBranch, isDepartment, isBatch } = this.state;
        ...
        {!isPageLoading && !isLoading && (
          <Fragment>
            ...
            <Container fluid>
              ...
              <div className="d-flex justify-content-end">{this.adduserNavigation()}</div>
                  ...
                  <div>
                    <section>
                      <Row>
                        ...
                        {isDepartment && (
                          <Col sm={12} md={3}>
                            <CustomSelect field="department" label="Department" name="department" getOptionValue={(option) => option.code} getOptionLabel={(option) => option.name} options={departmentIds} validateOnBlur validate={(e) => this.validateProperty("departmentId", e)} onChange={this.handleChange} />
                          </Col>
                        )}
                        {isBatch && (
                          <Col sm={6} md={3}>
            ...
            <Container fluid>{!isTableLoading && this.renderListForm(listType, tableData)}</Container>
          </Fragment>
        )}
      </Fragment>
    );
