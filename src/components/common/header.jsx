...
import { Collapse, DropdownItem, DropdownMenu, DropdownToggle, Nav, Navbar, NavbarBrand, NavbarToggler, NavItem, NavLink, UncontrolledDropdown, Container, Modal, ModalBody, ModalHeader, FormGroup, Button } from "reactstrap";
...
export default class Header extends React.Component {
  ...
  notificationList = async () => {
    ...
    try {
      ...
      if (statusCode === 1) return await this.setState({ data: res.data.data });
  ...
  notificationData = () => {
    ...
    if (data.length === 0) {
      return <img src={notification} alt="" style={{ width: "100%" }} />;
    }
  ...
  render() {
    ...
    let name = "",
      userType = session && session.data && session.data.userType,
      profileImageUrl = "",
      logoUrl = "";
    switch (userType) {
      case "sadmin":
    ...
    return (
      <Fragment>
        <Navbar expand="md" className="bg-white nav-eco">
          ...
          <NavbarBrand href={`${subDirectory}/dashboard`} className="">
          ...
          <Collapse isOpen={false} navbar>
            <Nav className="ml-auto" navbar>
              <NavItem>
                <NavLink href="#" onClick={this.toggleNotificationBar}>
                  {" "}
                  <FAIcons.FaBell />
                </NavLink>
              </NavItem>
              {session && session.uid && (
                <UncontrolledDropdown nav inNavbar>
                  ...
                  <DropdownMenu right>
                    <Link to={`${subDirectory}/dashboard`}>
                      <DropdownItem>User ID :{session.uid}</DropdownItem>
                    </Link>
                    <DropdownItem divider />
                    <DropdownItem>
                      <Link to={`${subDirectory}/${session.uid}/profile`}> Profile</Link>
                    </DropdownItem>
                    <DropdownItem divider />
                    <DropdownItem>
        ...
        {isNotifyDiv && <div className="notification-div">{this.notificationData()}</div>}
          ...
          <ModalBody>
            <Container>
              <Form getApi={this.setFormApi} onSubmit={this.onSubmit} style={{ width: "100%" }}>
                <Input field="password" label="Old Password" type="password" validateOnBlur validate={(e) => this.validateProperty("password", e)} readOnly={isFeildVisible} />

                {isFeildVisible && (
                  <div>
                    <Input field="newPassword" label="New Password" type="password" validateOnBlur validate={(e) => this.validateProperty("newPassword", e)} />
                    <Input field="confirmPassword" label="Confirm Password" type="password" validateOnBlur validate={(e) => this.validateProperty("confirmPassword", e)} />
                  </div>
                )}

                <FormGroup className="text-center">
                  <Button color="primary" type="submit" disabled={isLoading} style={{ width: "100% " }}>
                    {!isLoading ? btnName : <ClipLoader className="override" sizeUnit={"px"} size={25} color={"#ffffff"} loading={isLoading} />}
                  </Button>
                </FormGroup>
              </Form>
            </Container>
