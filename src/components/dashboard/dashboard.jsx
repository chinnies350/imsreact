import { Col, Row, Container, Progress, TabContent, TabPane, Nav, NavItem, NavLink } from "reactstrap";
...
export default class DashboardList extends Component {
  ...
  async initFun(data) {
    ...
    if (data && data.userLevel && data.userLevel !== "") userType = data.userLevel;
  ...
  userCountGeneratefun = async (data) => {
    const { userType, client, code, entity } = data;
    let clientData;
    ...
    switch (userType) {
      ...
      case "client":
        ...
        if (clientData.data.statusCode === 1) {
          ...
          let countObj = await this.countFun(_data);
          console.log(_data, countObj, "filter data");
          await this.setState({
            countObj,
          });
        } else {
      ...
      case "batch":
        ...
        if (clientData.data.statusCode === 1) {
          ...
          let _data = clientData.data.data;
          _data = await _.filter(_data, (v) => v.id === client);
          let countObj = await this.countFun(_data, code || entity);
          console.log(_data, countObj, "filter data", entity, code);
  ...
  countFun = async (_data, entity = null) => {
    let noClient = 0,
      noEntity = 0,
      noBranch = 0,
      noDepartment = 0,
    ...
    if (_data.length > 0) {
      ...
      if (entity === null) {
        ...
        await _data.map((d, i) => {
          // noEntity += d.Entity.length;
          noBranch += d.Branch.filter((brch) => {
            return brch.entity === entity;
          }).length;
          noDepartment += d.Department.filter((dept) => {
            return dept.entity === entity;
          }).length;
          noBatch += d.Batch.filter((batch) => {
            return batch.entity === entity;
          }).length;
          noStudent += d.Student.filter((stu) => {
            return stu.entity === entity;
          }).length;
          noStaff += d.Staff.filter((stf) => {
            return stf.entity === entity;
          }).length;
          return { noClient: noClient, noEntity: noEntity, noBranch: noBranch, noDepartment: noDepartment, noBatch: noBatch, noStudent: noStudent, noStaff: noStaff };
        });
      }
  ...
  feeCollectionDiv = () => {
    ...
    return (
      <div className="feeCollectionDiv tableCard todo">
        <h5>Fees</h5>
        ...
        <Progress multi>
          <Progress bar striped value={fees.noofTransaction}>
            {fees.noofTransaction}
          </Progress>
          <Progress bar striped color="info" value={fees.totalAmount}>
            {fees.totalAmount}
          </Progress>
          <Progress bar striped color="success" value={fees.collectAmount}>
            {fees.collectAmount}
          </Progress>
          <Progress bar striped color="danger" value={fees.dueAmount}>
            {fees.dueAmount}
          </Progress>
        </Progress>
      </div>
  ...
  currentdateTimetable = async (data) => {
    console.log("curent date timetable ");
  ...
  StaffTimetable = async (data) => {
    ...
    await data.forEach((v, i) => {
      if (Array.isArray(v[dayName])) {
        v[dayName].forEach((g) => {
          if (staff === g.staff) staffTimeData.push(g);
        });
      }
    });
  ...
  HolidayData = async (data) => {
    const { client, entity, branch, code, branchId } = data;

    try {
      let params = `client=${client || code}&entity=${entity || code}&branch=${branch || branchId}`;
  ...
  ScheduleType = async (data) => {
    const { client, entity, branch, department, batch, branchId, code } = data;

    try {
      let params = `client=${client || code}&entity=${entity || code}&branch=${branch || branchId}&department=${department}&batch=${batch}&type=event`;
      let res = await scheduleTypeId(params);
      if (res && res.data.statusCode === 1) {
  ...
  circulardata() {
    ...
    if (circularData.length > 0) {
      return circularData.map((_data, index) => {
        return (
          <tr className="circularcontentDiv">
            <td>
              <div className="d-flex align-items-center">
                ...
                <div>
                  ...
                  <div className="text-muted font-size-sm description">
                    <i className="icon-checkmark3 font-size-sm mr-1"></i>
                    {_data.description}
                  </div>
                </div>
              </div>
      ...
      return (
        <div className="circularcontentDiv">
              ...
              <Col sm={12}>
                <Row className="coursediv">
                  ...
                  <Col sm={12} style={{ textAlign: "right", margin: "0px", padding: "0px" }}>
                    <FAIcons.FaCalendarAlt style={{ color: "#607d8bba" }} />
                    <span className="text-muted font-size-sm date">
                      {_data.from.date} - {_data.to.date}
                    </span>
                  </Col>
                </Row>
              </Col>
  ...
  render() {
    ...
    console.log(";;;;; the data at dashboard", { data, TimeTable, TimetableData });
    let userType = data && data.userType;
    console.log("user typeeeeeeeeeeeee", userType);
    let toClientPage1, toClientPage2, toClientPage3, toClientPage4, toClientPage5;
    console.log('bbbbbbbbbbbbbbbbb')
    switch (userType) {
      case "sadmin":
        toClientPage1 = `${subDirectory}/client/list`;
        // toClientPage2 = `${subDirectory}/${data.client}/entity/list`;
        //* toClientPage2 = `${subDirectory}/entity/list`;
        toClientPage2 = `${subDirectory}/client/list`;

        toClientPage3 = `${subDirectory}/client/list`;
        toClientPage4 = `${subDirectory}/client/list`;
        toClientPage5 = `${subDirectory}/client/list`;
        break;
      case "client":
        toClientPage1 = `${subDirectory}/client/list`;
        toClientPage2 = `${subDirectory}/${data.client}/entity/list`;
        toClientPage3 = `${subDirectory}/${data.client}/entity/list`;
        toClientPage4 = `${subDirectory}/${data.client}/entity/list`;
        toClientPage5 = `${subDirectory}/${data.client}/entity/list`;
        break;
      case "entity":
        console.log('eeeeeeeeeeeeeeee')
        toClientPage1 = `${subDirectory}/client/list`;
        toClientPage2 = `${subDirectory}/dashboard`;
        //  *  toClientPage3 = `${subDirectory}/${data.client}/${data.code}/entity/list`;
        toClientPage3 = `${subDirectory}/dashboard`;
        //* toClientPage4 = `${subDirectory}/dashboard`;
        toClientPage4 = `${subDirectory}/settings/department`;
        // toClientPage4 = `${data.client}/${data.uid}/${data.branchId}/department/list`;
        // toClientPage5 = `${subDirectory}/dashboard`;
        toClientPage5 = `${subDirectory}/settings/batch`;
        console.log('tttttttttttttt',toClientPage4)
        break;
      case "branch":
        toClientPage1 = `${subDirectory}/client/list`;
        toClientPage2 = `${subDirectory}/dashboard`;
        toClientPage3 = `${subDirectory}/${data.client}/entity/list`;
        toClientPage4 = `${subDirectory}${data.client}/${data.entity}/${data.branch}/department/list`;
        toClientPage5 = `${subDirectory}/dashboard`;
        break;
      default:
        break;
      ...
      <Fragment>
        {data && data.userType !== "student" && data.userType !== "staff" && data.userType !== "parent" && (
          <div className="dashboardflexdiv">
            {/* Client as Group */}
            {data.userType !== "client" && data.userType !== "entity" && <DashboardItem value={countObj && countObj.noClient} title="Total Group" icon="fa fa-list-alt" props={this.props} toLink={toClientPage1} className="myCard todo" iconclass="iconstyle iconbgclr1" />}
            {/* Entity as Instuitution */}
            {data.userType !== "entity" && (
              <>
                <DashboardItem value={countObj && countObj.noEntity} title="Total Institution" icon="fa fa-list-ul" props={this.props} toLink={toClientPage2} className="myCard todo" iconclass="iconstyle iconbgclr2" />
            ...
            <DashboardItem value={countObj && countObj.noStaff} title="Total Staffs" icon="fa fa-users" props={this.props} toLink={`${subDirectory}/staffs`} className="myCard todo" iconclass="iconstyle iconbgclr6" />

            <DashboardItem value={countObj && countObj.noStudent} title="Total Students" icon="fa fa-users" props={this.props} toLink={`${subDirectory}/users`} className="myCard todo" iconclass="iconstyle iconbgclr7" />
          </div>
        )}
        <Container fluid>
          ...
          <div>
            <Row>
              <Col sm={12}>
                ...
                {feeCollectionDiv && <section>{this.feeCollectionDiv()}</section>}
              </Col>
            </Row>
          </div>
        </Container>
