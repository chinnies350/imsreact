...
export default class ExamList extends Component {
  ...
  adduserNavigation() {
    ...
    if (form === 'section')
      return <div className="btn btn-primary btn-sm" onClick={() => this.editFun(`${subDirectory}/exam/add/section`, state)}>+ Add Section</div>
    if (form === 'viewQuestions' && data.length === 0)
      return <div className="btn btn-primary btn-sm" onClick={() => this.editFun(`${subDirectory}/exam/add/questionForm`, state)}>+ Add Question</div>
    return '';
  }
  ...
  getColumnHeaders(type, prefixUrl = "", dynamicLabels = {}) {
    ...
    let def = {
      ...
      "Client": {
        dataField: 'client', text: `GroupName`, filter: this.getTextFilter(), sort: true
      },
      "Entity": { dataField: 'entityName', text: `Institution`, filter: this.getTextFilter(), sort: true },
  ...
  offlineactionsFormatter = (cell, row, rowIndex, formatExtraData) => {
    ...
    let _form = "Offline Exam";
    let links = [];
    rightsData && rightsData[_form] && rightsData[_form].view.value &&
      links.push(<div onClick={() => this.editFun(`${subDirectory}/exam/view/exam`, row)} className='badge badge-success' > View</div >)
    rightsData && rightsData[_form] && rightsData[_form].edit.value &&
      links.push(<div onClick={() => this.editFun(`${subDirectory}/exam/edit/exam`, row)} className='badge badge-warning'>Edit</div>)
    ...
    rightsData && rightsData[_form] && rightsData[_form].edit.value &&
      links.push(<div onClick={() => this.editFun(`${subDirectory}/exam/section`, row)} className='badge badge-success'>View Section</div>)
    rightsData && rightsData[_form] && rightsData[_form].edit.value &&
      links.push(<div onClick={() => this.editFun(`${subDirectory}/exam/view/previewQuestion`, row)} className='badge badge-dark'>Preview Question</div>)
    rightsData && rightsData[_form] && rightsData[_form].edit.value &&
      links.push(<div onClick={() => this.editFun(`${subDirectory}/exam/view/previewAnswer`, row)} className='badge badge-blue'>View Answer Key</div>)
  ...
  onlineactionsFormatter = (cell, row, rowIndex, formatExtraData) => {
    ...
    if (userType === "student") {
      if (sessionData.client === row.client && sessionData.entity === row.entity && sessionData.branch === row.branch && sessionData.department === row.departmentId) {
        links.push(<div onClick={() => this.onlineExamAttend(`${subDirectory}/onlineExam`, row)} className='badge badge-danger'>Attend</div>)
      }
    } else {
  ...
  sectionactionsFormatter = (cell, row, rowIndex, formatExtraData) => {
    ...
    let _form = "Offline Exam";
    rightsData && rightsData[_form] && rightsData[_form].edit.value &&
      links.push(<div onClick={() => this.editFun(`${subDirectory}/exam/view/section`, row)} className='badge badge-success'>View</div>)
    rightsData && rightsData[_form] && rightsData[_form].edit.value &&
      links.push(<div onClick={() => this.editFun(`${subDirectory}/exam/edit/section`, row)} className='badge badge-warning'>Edit</div>)
    rightsData && rightsData[_form] && rightsData[_form].delete.value &&
      links.push(<div onClick={() => this.SessiondeleteFun(row)} className='badge badge-danger'>Delete</div>)
    rightsData && rightsData[_form] && rightsData[_form].edit.value &&
      links.push(<div onClick={() => this.editFun(`${subDirectory}/exam/viewQuestions`, row)} className='badge badge-success'>View Qestions</div>)
    return <div className="actions">{links.concat(" ")}</div>
  }
  ...
  qusActionsFormatter = (cell, row, rowIndex, formatExtraData) => {
    ...
    const { rightsData } = this.state;
    rightsData && rightsData[_form] && rightsData[_form].edit.value &&
      links.push(<div onClick={() => this.quesionEdit(`${subDirectory}/exam/edit/questionForm`, row)} className='badge badge-warning'>Edit</div>)
  ...
  noofTimesCountFun = async (row) => {
    ...
    try {
      ...
      console.log(res,params,this.state)
      if (res.data.statusCode === 1) {
        let data = res.data.data;
        if (userType === 'student')
  ...
  render() {
    ...
    let _form = form;
    if(_form === "section") _form = "Offline Exam"
    if(_form === "viewQuestions") _form = "Offline Exam"
