...
export default class CircularList extends Component {
  ...
  getColumnHeaders(type, prefixUrl = "", dynamicLabels = {}) {
    ...
    let def = {
      ...
      "Client": {dataField: 'client', text: `Group`, filter: this.getTextFilter(), sort: true},
  ...
  actionFormater = (cell, row, rowIndex, formatExtraData) => {
    let links = []
    links.push(<div onClick={() => this.editFun(`${subDirectory}/notification/view/circular`, row)} className='badge badge-success'>View</div>)
    links.push(<div onClick={() => this.editFun(`${subDirectory}/notification/edit/circular`, row)} className='badge badge-warning'>Edit</div>)
