...
import { saveClient, updateClient, addInstituteEntity, updateInstituteEntity } from 'services/clientService'
...
class ClientEdit extends Component {
  ...
  schema = {
    ...
    addremail: Joi.string().email().required().label("Addremail"),
  ...
  async componentDidMount() {
    const { match: { params: { institutionaction, type } }, session: { data: sessionData }, location: { state: { entityId, funType } } } = this.props;

    console.log(";;;;;;;;;;;;;;;;;;;;;;;; the props at did mount ", this.props)
    let sampleData = {};
    if (funType === "add") {
      sampleData = await this.addNewClient();
      await this.formApi.setValues(sampleData);
    } else {
      sampleData = await this.getSampleData(entityId);
      await this.formApi.setValues(sampleData);
    }
  ...
  getSampleData = async (entityId) => {
    ...
    // console.log(";;;;;;;;;;;;;;;;;;;;;;;;;;;; the row at institute ", row)
    // if (r.data.statusCode === 1) {
    //   r = r.data.data[0];
    //   const { name } = r; 
    //   console.log(";;;;;;;;;;;;;;;;;;;;;;;;;;;; the name at institute ", name)
    //   data['entity'] = name.filter(v=> v.uid === entityId );
    //   console.log(";;;;;;;;;;;;;;;;;;;;;;;;;;;; the data at institute ", data)
    //   return data;
    // } else {
    //   return data
    // }
    ...
    let { location: { state: { row } } } = this.props;
    row['ref'] = { client: [], entity: [], branch: [], department: [], branch: [] }
  ...
  saveEntity = async (data) => {
    console.log(";;;;;;;;;;;;;;;;;;;;;;;;;;; the data at saveEntity ", data)
    var res = await addInstituteEntity(data["entity"][0]);
  ...
  updateEntity = async (data) => {
    console.log(";;;;;;;;;;;;;;;;;;;;;;;;;;; the data at updateEntity ", data)
  ...
  //   const { session: { data: { clientId,uid:userId } } } = this.props;   
  //   data["code"] = this.props.location.state.clientId
  //   // var res = await updateClient(userId,data); // this line was active before
  //   const { data: { statusCode } } = res;
  //   if (statusCode !== 1) return alert('Something went wrong.Please try again later.');
  //   this.redirectTo();
  // }
  ...
  onSubmit = async () => {
    ...
    const { entityImages } = this.state
    const { entity } = data
    const { location: { state: { entityId, funType } } } = this.props;
    for (let i = 0; i <= data["entity"].length - 1; i++) {
      data["entity"][0]["type"] = this.props.session.data.userType
      data["entity"][0]["client"] = this.props.location.state.clientId
    }
  ...
  render() {
    ...
    const { location: { state: { entityId, funType, clientId } } } = this.props;

    let ent_url = `${subDirectory}/${clientId}/entity/list`
        ...
        {!isPageLoading &&
          <Fragment>
            <Breadcrumb>
              ...
              <BreadcrumbItem><NavLink to={ent_url}>Institution</NavLink></BreadcrumbItem>
              {funType === "add" &&
                <BreadcrumbItem active>Add</BreadcrumbItem>}
              {funType !== "add" &&
                <Fragment>
                  <BreadcrumbItem active>{entityId}</BreadcrumbItem>
                      ...
                      <Scope scope={`entity[${i}]`} key={i}>
                        ...
                        <Row>
                          <Col sm={6} md={2}>
                                       {/* Entitiy as Institution */}
                            <Input
                              field="code" label="Institution Code"
                              validateOnBlur validate={e => this.validateProperty('code', e)}
                            />
                          </Col>
                          <Col sm={12} md={4}>
                                       {/* Entitiy as Institution */}
                            <Input
                              field="name" label="Name of the Institution*"
                              validateOnBlur validate={e => this.validateProperty('name', e)}
                            />
                          </Col>
                        ...
                        <Row>
                          <Col sm={12} md={6}>
                            <Input
                              field="logo" label="Institution Logo"
                              validateOnBlur validate={e => this.validateProperty('logo', e)}
                            />
                            <PreviewImage
