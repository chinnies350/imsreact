...
export default class ViewClient extends Component {
  ...
  editClientsDetails(row) {
    ...
    self.props.history.push({
      pathname: `${subDirectory}/main/add-client`,
      state: {
        clientData: row,
        isEditForm: true
      }
  ...
  sendData(row) {
    let self = this;
    self.props.history.push({
      pathname: `${subDirectory}/main/view-client-details`,
      state: {
        ViewclientData: row
      }
    });
  ...
  render() {
    ...
    return (
      <Container fluid>
        ...
        <Row>
          <Col sm="3">
            <TabItem icon={<FAIcons.FaUserPlus />} text={"Add Institution"} to={`${subDirectory}/main/add-client`} />
          </Col>
          <Col sm="3">
