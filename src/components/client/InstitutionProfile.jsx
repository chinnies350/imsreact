...
import { saveClient, updateClient, updateInstituteEntity } from 'services/clientService'
...
class ClientEdit extends Component {
  ...
  async componentDidMount() {
    ...
    // info["entity"] = sampleData
    await this.formApi.setValues(info);
  }


  getSampleData = async () => {
    const { session: { data } } = this.props;
    let r = await getClients(`type=client&client=${data["client"]}`)
    // let data = {};
    // data['client'] = client;
    // data['code'] = client;
    // data['uid'] = client;
    ...
    if (Array.isArray(data["address"])) {
      ...
      data["academic"] = data["academic"][0]
      data["admission"] = data["admission"][0]
      data["vacation"] = data["vacation"][0]
    }
    data["password"] = data["defaultpassword"]
    // if (r.data.statusCode === 1) {
    //   r = r.data.data[0];
    //   const { name } = r;
    //   data['entity'] = name.filter(v=> v.uid === data['code'] );
    //   return data;
    // } else {
    return data
    // }
  ...
  updateEntity = async (data) => {
    var res = await updateInstituteEntity(data["entity"][0]); // new line added on 22/6/19
    // var res = ''
    const { data: { statusCode, message } } = res;
    // if (statusCode === 1) {
    //   this.props.session.data = data["entity"][0]
    // };
    // alert(message);
    if (statusCode !== 1) {
      return  ToastService.Toast(message,"default")
  ...
  redirectTo = async () => {
    // const { data: { type, code } } = this.state;
    const { session: { uid, data: { type } } } = this.props
    let url = '';
    switch (type) {
      case "client":
  ...
  handleChange = async (field, { target: { files } }) => {
    const values = this.formApi.getState().values;
    let imageUrl = await FileUpload(files[0]).then((res) => res.data.name)
    await this.setState({ logo: imageUrl });
    values.entity[0]["logo"] = imageUrl
    await this.formApi.setValues({ ...values });
  }
        ...
        {!isPageLoading &&
          <Fragment>
            <Breadcrumb>
              ...
              <BreadcrumbItem><NavLink to={`${subDirectory}/client/list`}>Institution</NavLink></BreadcrumbItem>
            ...
            <Form getApi={this.setFormApi} onSubmit={this.onSubmit}>
              ...
              {({ formApi, formState }) => (
                <div>
                  ...
                  {console.log(":;;;;;;;;;;;;;;; the formstate at profile ", formState)}
                  <section>
                    <h6>
                      ...
                      <Scope scope={`entity[${i}]`} key={i}>
                        ...
                        <Row>
                          <Col sm={6} md={2}>
                            <Input
                              ...
                              validateOnBlur validate={e => this.validateProperty('code', e)} readOnly
                            />
                          </Col>
                          <Col sm={12} md={4}>
                            <Input
                        ...
                        <Row>
                          <Col sm={6} md={2}>
                            <Input
                              ...
                              validateOnBlur validate={e => this.validateProperty('branchId', e)} readOnly
                            />
                          </Col>
                          <Col sm={12} md={4}>
                            <Input
                        ...
                        <Row>
                          <Col>
                            <h6>
                              {/* Entity as Institution */}
                              <span>Institution Credentials</span>
                            </h6>
                          </Col>
                        </Row>
                        <Row>
                          ...
                          <Col sm={12} md={4}>
                            <Input
                              field="password" label="Password*"
                              validateOnBlur validate={e => this.validateProperty('password', e)} readOnly
                            />
                          </Col>
                        </Row>
                        ...
                        <Row>
                          <Col sm={12} md={4}>
                            ...
                            <DRP1 field="admission" label="Admission Period" id="addmission"
                             startDate={formState.values&&formState.values.entity[i]&&formState.values.entity[i].admission?moment(formState.values.entity[i].admission.from):null}
                              endDate={formState.values&&formState.values.entity[i]&&formState.values.entity[i].admission?moment(formState.values.entity[i].admission.to):null} onChange={(data) => this.dateValue(data, i, "admission")} validate={e => this.validateProperty('admission', e)} />
                          </Col>
                          <Col sm={12} md={4}>
                            <label>Acadamic*</label>
                            <DRP1 field="academic" label="Academic Period" id="academic"
                             startDate={formState.values&&formState.values.entity[i]&&formState.values.entity[i].academic?moment(formState.values.entity[i].academic.from):null}
                              endDate={formState.values&&formState.values.entity[i]&&formState.values.entity[i].academic?moment(formState.values.entity[i].academic.to):null} onChange={(data) => this.dateValue(data, i, "academic")} validate={e => this.validateProperty('academic', e)} />
                          </Col>
                          <Col sm={12} md={4}>
                            <label>Vacation*</label>
                            <DRP1 field="vacation" label="Vacation Period" id="vacation"
                             startDate={formState.values&&formState.values.entity[i]&&formState.values.entity[i].vacation?moment(formState.values.entity[i].vacation.from):null} 
                             endDate={formState.values&&formState.values.entity[i]&&formState.values.entity[i].vacation?moment(formState.values.entity[i].vacation.to):null} onChange={(data) => this.dateValue(data, i, "vacation")} validate={e => this.validateProperty('vacation', e)} />
                          </Col>
                        </Row>
                        <br />
                        <section>
