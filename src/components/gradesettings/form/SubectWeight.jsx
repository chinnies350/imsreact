...
export default class SubjectWeight extends Component {
  ...
  schema = {
    ...
    creditPoint: Joi.number().min(1).required().label("Credit Point"),
  };
  ...
  onSubmit = async () => {
    console.log(' subject weightage submit')
    ...
    try {
      ...
      if (statusCode === 1) {
        ...
        let nextPage = `${subDirectory}/grade/subjectWeightage/${data.client}/${data.entity}/${data.branch}/${data.departmentId}`
        console.log(";;;;;;refdsfgdfgf;;;;;;;;;;;; ", nextPage)
        await this.nextPage(data)
        console.log(data)
      } else {
        return ToastService.Toast(message ||"Somthing went wrong.Please try again later", "default");
      }
    } catch (err) {
      this.handleError(err)
    }
  ...
  nextPage = async (data) => {
    console.log('nxtpageeeeeee',this.props.props)
    this.props.props.history.push({
        pathname: `${subDirectory}/grade/subjectWeightage`,
        // state:data,
        state: {
            "data": data,
  ...
  render() {
    const { clientIds, entityIds, branchIds, departmentId,departmentIds, formActionType,isClient, isEntity, isBranch, isDepartment } = this.state
    const { actionType } = this.props
    console.log(this.state)
    return (
      <Fragment>
        <Container fluid>
          <Breadcrumb>
            ...
            <BreadcrumbItem><NavLink to={`${subDirectory}/grade/subjectWeightage`}>Subject Weightage</NavLink></BreadcrumbItem>
            <BreadcrumbItem active>{actionType} Subject Weightage</BreadcrumbItem>
          </Breadcrumb>
          <div className="mb-4">
            <h6>{actionType} Subject Weightage</h6>
          </div>
                      ...
                      <Row>
                        ...
                        <Col sm={12} md={4}>
                          <Input
                            ...
                            validateOnBlur
                      validate={(e) => validateProperty(true, "numberRange", e, "Credit Point",{min:0,max:1000})}
