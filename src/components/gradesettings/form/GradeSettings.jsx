...
export default class GradeSettings extends Component {
  ...
  validateProperty = (name, value) => {
    ...
    console.log(this.formApi.getState().values.markFrom,"mark",this.schema)
  ...
  schema = {
    ...
    grade: Joi.string().required().label("Grade Name"),
    gradePoints: Joi.number().min(1).required().label("Grade Points"),
    markFrom: Joi.number().min(1).required().label("Mark Start Range"),
    // markTo: Joi.number().min(1).required().label("Mark End Range"),
    markTo: Joi.number().min(1).required().label("Mark End Range"),
  };
      ...
      <Fragment>
        <Container fluid>
          <Breadcrumb>
            ...
            <BreadcrumbItem><NavLink to={`${subDirectory}/grade/grade`}>Grade</NavLink></BreadcrumbItem>
              ...
              <div>
                ...
                <section>
                  <Row>
                    ...
                    <Col sm={6} md={3}>
                      <Input field="gradePoints" label="Grade Points*"
                       validateOnBlur 
                      //  validate={e => this.validateProperty('gradePoints', e)} 
                      validate={(e) => validateProperty(true, "numberRange", e, "Grade Points",{min:0,max:100})}

                       readOnly={formActionType} />
                    </Col>
                    <Col sm={6} md={3}>
                      <Input field="markFrom" label="Mark Start Range*" 
                      validateOnBlur
                      //  validate={e => this.validateProperty('markFrom', e)}
                      validate={(e) => validateProperty(true, "numberRange", e, "Grade Points",{min:0,max:1000})}

                        readOnly={formActionType} />
                    </Col>
                    <Col sm={6} md={3}>
                      <Input field="markTo" label="Mark End Range*" 
                      // validateOnBlur validate={e => this.validateProperty('markTo', e)}
                      validateOnBlur
                      validate={(e) => validateProperty(true, "numberRange", e, "Mark Start Range",{min:parseInt(formState.values.markFrom)||1,max:1000})}
                       readOnly={formActionType} />
                    </Col>
                  </Row>
                </section>
