...
import { Breadcrumb, BreadcrumbItem, Collapse, DropdownItem, DropdownMenu, DropdownToggle, UncontrolledButtonDropdown, Container, Row, Col, Modal, ModalBody, ModalHeader } from "reactstrap";
...
import { CustomSelect, SDP, Textarea } from "components/common/forms";
import { getselectData, saveUsersExcel, deleteUser } from "services/userService";

import { subDirectory } from "../../config.json";
import { Button } from "bootstrap";
import { transferStudent } from "../../services/transferServices";
import { CustomDatePicker } from "../common/forms";
...
export default class TransferList extends Component {
  ...
  async componentDidMount() {
    ...
    console.log(":;;;;;;;;;;;;;;;;;;;;;; datassss", data, "props", this.props);
    ...
    if(this.props.TransferType=="Graduation"){
      await this.dateValue(moment(this.state.date));
      // this.formApi.setValue("date",moment(this.state.date));
    }
  }
  ...
  handleOnSelect = (row, isSelect) => {
    if (isSelect) {
      this.setState(() => ({
        selected: [...this.state.selected, row.uid],
        TransferData: [...this.state.TransferData, { studentId: row.uid, batch: row.batch }],
      }));
    } else {
      this.setState(() => ({
        selected: this.state.selected.filter((x) => x !== row.uid),
        TransferData: this.state.TransferData.filter((x) => x.uid !== row.uid),
      }));
    }
  };
  handleOnSelectAll = (isSelect, rows) => {
  ...
  transferData = async () => {
    const { batch, branch, client, department, entity, uid, userType } = this.props.formData;
    const inputData = this.formApi.getState().values;
    console.log(this.state.TransferData, "transfer data", inputData);

    if (this.state.TransferData.length > 0) {
   
      if (this.props.TransferType == "Transfer") {
        if (batch == inputData.batch) {
          return ToastService.Toast("Select Diffrent Batch.", "default");
        }
        let studentData = this.state.TransferData.map((student) => {
          student["batch"] = inputData.batch;
          return student;
        });
      ...
      } else if (this.props.TransferType == "Graduation") {

        let data = {
          queryType: "graduation",
          client: client,
          entity: entity,
          branch: branch,
          department: department,
          ...
          studentList: [...this.state.TransferData],
        };
        console.log(data);
        const res = await transferStudent(data);
      ...
      // }
        ...
        // ToastService.Toast("Graduation", "default");
      }
    } else {
    
      ToastService.Toast("Select Student", "default");
    }
  ...
  // adduserNavigation = async (props) => {
  //   props.history.push({
  //     pathname: `${subDirectory}/user/register/new`,
  //     state: {
  //         funType: "add",
  //         addType:"student"
  //     }
  ...
  clientDatas = async (name) => {
    ...
    switch (name) {
      case "client":
        this.selectoptGet(`namelist?client=${formApiData.client}&type=client`, "entityIds");
        await this.setState({
          entity: "",
          branch: "",
          department: "",
        ...
        break;
      case "entity":
        this.selectoptGet(`namelist?client=${formApiData.client}&type=entity&entity=${formApiData.entity}`, "branchIds");
        await this.setState({
          branch: "",
          department: "",
          batch: "",
        ...
        break;
      case "branch":
        this.selectoptGet(`namelist?client=${formApiData.client}&type=branch&entity=${formApiData.entity}&branch=${formApiData.branch}`, "departmentIds");
        await this.setState({ department: "", batch: "", batchIds: [] });
        break;
      case "department":
        this.selectoptGet(`namelist?client=${formApiData.client}&type=department&entity=${formApiData.entity}&branch=${formApiData.branch}&department=${formApiData.department}`, "batchIds");
  ...
  getColumnHeaders(type, prefixUrl = "", dynamicLabels = {}) {
    let labels = dynamicLabels["user"] ? dynamicLabels : this.getDefaultClientLabels();
    ...
    switch (type) {
      ...
      case "batch":
        ...
        break;
      case "uid":
        excludeKeys = ["Client", "Entity", "Branch", "Department", "Batch", "UserID"];
        break;
      default:
        break;
    }
  ...
  //     links.push(
  //       <NavLink
  //         to={`${subDirectory}/${row.client}/${row.entity}/${row.branch}/${row.uid}/profile`}
  ...
  //     links.push(
  //       <NavLink
  //         to={`${subDirectory}/${row.client}/${row.entity}/${row.branch}/${
  //           row.uid
  //           }/edit/login`}
  ...
  render() {
    ...
    const { parentData, props } = this.props;
    const selectRow = {
      mode: "checkbox",
          ...
          <React.Fragment>
            {/* <Breadcrumb>
              ...
              {parentData &&
                parentData.map(d => (
                  <BreadcrumbItem>
                    <NavLink to={`${subDirectory}/${d.type}/${d.id}`}>{d.id}</NavLink>
                  </BreadcrumbItem>
                ))}
            ...
            <div className="d-md-flex align-items-md-center justify-content-md-between maincontentheader">
              ...
              {/* <div> */}
              {/* <button
                  className="btn btn-outline-info btn-sm ml-auto"
            ...
            <Collapse isOpen={this.state.toggleColumns}>
              <div className="alert alert-info alert-sm">
                ...
                {colKeys.map((k, i) => {
                  ...
                  return (
                    <div key={`toggle_${k}`} className="custom-control custom-control-inline col-6 col-md-2 custom-checkbox">
                      <input type="checkbox" className="custom-control-input" checked={this.isColumnVisible(k)} onChange={(e) => this.toggleColumn(i)} id={`toggle_${k}`} />
                      <label className="custom-control-label" htmlFor={`toggle_${k}`}>
                        {colDef[k]["desc"] ? colDef[k]["desc"] : colDef[k]["text"]}
                      </label>
                    </div>
                  );
                })}
            ...
            {!isLoading && (
              <>
                ...
                <hr />
                <Form getApi={this.setFormApi} onSubmit={() => this.transferData()}>
                  {({ formApi, formState }) => (
                    <div className="d-flex">
                      {this.props.TransferType == "Transfer" && (
                        <>
                          <div className="mr-2 ">
                            <Col md={12}>
                              <CustomSelect
                      ...
                      {this.props.TransferType == "Graduation" && (
                        <>
                          <Col md={3}>
                            <CustomDatePicker
                              field="date"
                              ...
                              date={moment(date)}
                              validate={(e) => this.validateProperty("date", e)}
                              onChange={this.dateValue}
                              onBlur={(e) => this.validateProperty("date", e)}
                              numberOfMonths={1}
                            />
                          </Col>
                          <Col>
                            <div className="ml-3 mr-3">
                              {" "}
                              <Textarea field="remarks" label="Remarks" name="remarks" />
                            </div>
                          </Col>
                          <Col>
                            <button className="btn btn-primary btn-sm ml-3 mt-4" type="submit">
                              Graduate
                            </button>
                          </Col>{" "}
                        </>
                  ...
                  //  Client as Group  and Entity as Institution
                        ...
                        <Row className="justify-content-end">
                          <button
                            ...
                            onClick={this.bulkModalToggle}
                            style={{marginLeft:'31px', marginRight: "20px" }}
                          >
                            Cancel
...
//   actionsFormatter = (cell, row, rowIndex, formatExtraData) => {
//     const { rightsData } = this.state;
//     const { sessionData: { userType } } = this.props;
...
//     }
//     if (userType === 'student' && row.status === 'request' ) {
//       links.push(<div onClick={() => this.updateLeave(row, 'Cancelled')} className='badge badge-danger' style={{ cursor: 'pointer' }}>Cancel</div>)
//     }
//     if (row.status === 'Cancelled') {
//       links.push(<div className='badge badge-warning'>Cancelled</div>)
//     }
...
//     let res = await updateLeaves(params)
//     if (res.data.statusCode === 1) {
//       ToastService.Toast(`Leave Cancelled Successfully `, "default")
//       refreshTable()
//     } else {
//       ToastService.Toast(`Leave Cancel Failed!!!`, "default")
//     }
