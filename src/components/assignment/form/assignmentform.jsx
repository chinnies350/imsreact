...
export default class AddAssignment extends Component {
  constructor(props) {
    ...
    this.state = {
      ...
      data1: '',
      todayDate1: new Date().toISOString().slice(0, 10),
      todayDate2: new Date().toISOString().slice(12, 19),
      todayDate: "{' '}",
      client: '',
      department: '',
      isEditForm: false,
  ...
  async componentDidMount() {
    ...
    // var todayDate2 = new Date().toISOString().slice(12, 19);

    var todayDate = this.state.todayDate1 + " " + this.state.todayDate2
    console.log(todayDate)
    await this.setState({
      // todayDate: todayDate,

      todayDate: this.state.todayDate1 + " " + this.state.todayDate2,
  ...
  async getAssignmentsList() {
    ...
    let params = `client=${client}&entity=${entity}&branch=${branch}&departmentId=${this.state.data1.department}&batchId=${batch}&subject=${subject}&type=assignment`;
    try {
      const assignmentList = await getAssignmentList(params)
      console.log(assignmentList, params)
  ...
  async  particularAssignment() {
    ...
    console.log(particularDetails, params)
  ...
  getStudentList = async () => {
    ...
    try {
      ...
      console.log(";;;;;;;;;;;;;;;;;;;; dataaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa", res)
  ...
  async payloadData(data) {
    ...
    console.log(addedDetails, temp)
    if (addedDetails.data.statusCode === 1) {
      ToastService.Toast(addedDetails.data.Message, 'default');
      this.props.props.history.push(`${subDirectory}/assignments/assignment`)
    }
  }
  ...
  async updateAssignment(data) {
    ...
    if (this.state.storeStudentMarks !== undefined) {
      ...
      let edittemp = {
        "date": this.state.todayDate,
        "createdDate": this.state.todayDate,
        "batch": formdata.batch,
        // "department": formdata.department,
        "department": this.state.data1.department,
        "assignmentId": this.state.assignmentId,
        "client": formdata.client,
        "entity": formdata.entity,
        "branch": formdata.branch,
      ...
      console.log(edittemp, this.state)
      const updateDetails = await editAssignments(edittemp)
      console.log(updateDetails, edittemp)
  ...
  render() {
    ...
    var editarray = []
    const { data, data1, clientIds, entityIds, branchIds, departmentIds, batchIds, studentList, isTableLoading, editValues, isClient, isEntity, isBranch, isDepartment, isBatch } = this.state;
        ...
        }
        {
          action === 'edit' && editValues ?
            <section>
              ...
              <Row>
                <Col md={12} className="d-flex justify-content-end">
                  <button type="submit" onClick={() => this.oneditSubmit()} className="btn btn-primary btn-sm">Submit</button>
                </Col>
              </Row>
