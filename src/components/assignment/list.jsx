...
export default class AssignmentList extends Component {
    ...
    actionsFormatter = (cell, row, rowIndex, formatExtraData) => {
        ...
        rightsData && rightsData[_form] && rightsData[_form].edit.value &&
            links.push(<div onClick={() => this.editFun(`${subDirectory}/assignments/edit/${type}`, row)} className='badge badge-warning'>Edit</div>)
    ...
    async tableData() {
        ...
        if (type === 'homework') {
            let columns = [
                ...
                { dataField: 'status', text: 'Status' ,filter: this.getTextFilter()},
    ...
    render() {
        const { homeworkView, columns, rightsData,data,client,entity,branch } = this.state
...
async function actionSetting(type, row, data) {
    ...
    if (type === 'assignment') {
        ...
        if (deleteAss.data.statusCode === 1) {    
            setTimeout(`location.href = '${subDirectory}/assignments/assignment';`);    
        ...
        if (deletehw.data.statusCode === 1) {     
            setTimeout(`location.href = '${subDirectory}/assignments/homework';`);         
