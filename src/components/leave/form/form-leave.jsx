...
import { leaveInsert, staffListLeave } from 'services/leaveService';
...
to:new Date().toLocaleDateString(),
    ...
    schema = {
        ...
        batch: Joi.any().required().label("Batch"),
    ...
    clientDatas = async (name, Input) => {
        ...
        switch (name) {
            ...
            case "department":
                ...
                await this.staffListLeave(`type=${data.userType}&client=${data.client}&type=department&entity=${data.entity}&branch=${data.branch}&department=${data.department}`)
    ...
    onSubmit = async () => {
        ...
        if (res.data.statusCode === 1) {
            this.props.props.history.push({
                pathname: `${subDirectory}/leave/leave`,
            })
    ...
    render() {
        const { date,clientIds, entityIds, branchIds, departmentIds, batchIds, isClient, isEntity, isBranch, isDepartment, isBatch,staffList } = this.state
