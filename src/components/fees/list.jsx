...
export default class FeesList extends Component {
  ...
  adduserNavigation() {
    ...
    if (feesType === 'feeallocation' && feesType === 'feecollection')
      return <div className="btn btn-primary btn-sm" onClick={() => this.paymentFun(`${subDirectory}/fees/add/feeallocation`, state)}>+ Add Fee Allocation</div>
    if (feesType === 'feecollection' && data.length === 0)
      return <div className="btn btn-primary btn-sm" onClick={() => this.paymentFun(`${subDirectory}/fees/add/feecollection`, state)}>+ Add Fee Collection</div>
  ...
  allocationactionsFormatter = (cell, row, rowIndex, formatExtraData) => {
    ...
    let _form = feesType;
    if (_form === 'feeallocation') _form = "Fee Allocation"
    if (_form === 'feecollection') _form = "Fee Collection"
    let links = []
    rightsData && rightsData[_form] && rightsData[_form].edit.value &&
      links.push(<div onClick={() => this.paymentFun(`${subDirectory}/fees/edit/feeallocation`, row)} className='badge badge-warning'>Edit</div>)
  ...
  collectionactionsFormatter = (cell, row, rowIndex, formatExtraData) => {
    ...
    {
      links.push(<div onClick={() => this.paymentFun(`${subDirectory}/fees/add/feecollection`, row)} className='badge badge-success'>Pay</div>)
    }
  ...
  deleteRow = async (row) => {
    ...
    if (response.data.statusCode === 1) {
      ...
      setTimeout(`location.href = '${subDirectory}/fees/feeallocation';`);
      this.initTableData()
    }
  }
