...
import { getStaffAllLeaves } from 'services/leaveService'
...
export default class leave extends Component {
  ...
  rightsData = async (session) => {
    let res = await rightsData("staffs", session);
    let excludeModules = [];
  ...
  renderLeaveForm(data) {
    const { rightsData, data: sessionData } = this.state;
    return <LeaveList data={data} rightsData={rightsData} sessionData={sessionData} refreshTable={this.onSubmit} />
  }
  ...
  onSubmit = async () => {
    ...
    console.log(params)
    const response = await getStaffAllLeaves(params) 
              ...
              <div className="d-md-flex align-items-md-center justify-content-md-between maincontentheader">
                ...
                <div  >
                  {rightsData && rightsData[_form] && rightsData[_form].create.value &&
                    <NavLink className="btn btn-primary btn-sm" to={`${subDirectory}/staffleave/add/${leaveType}`}>+ {leaveType} </NavLink>
                  }
                </div>
              </div>
