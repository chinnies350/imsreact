...
import { staffUpdateLeaves } from "services/leaveService";
...
export default class LeaveList extends Component {
  ...
  getColumnHeaders(prefixUrl = "") {
    ...
    let def = {
      ...
      fromDate: { dataField: "fromDate", text: "From Date", sort: true },
  ...
  actionsFormatter = (cell, row, rowIndex, formatExtraData) => {
    ...
    if (userType !== "student" && userType !== "parent") {
      rightsData &&
        ...
        row.status === "request" &&
        links.push(
          <div onClick={() => this.updateLeave(row, "Approved")} className="badge badge-success" style={{ cursor: "pointer" }}>
            Approve
          </div>
        );
      rightsData &&
        ...
        row.status === "request" &&
        links.push(
          <div onClick={() => this.updateLeave(row, "Rejected")} className="badge badge-danger" style={{ cursor: "pointer" }}>
            Reject
          </div>
        );
    }
    if (userType === "student" && row.status === "request") {
      links.push(
        <div onClick={() => this.updateLeave(row, "Cancelled")} className="badge badge-danger" style={{ cursor: "pointer" }}>
          Cancel
        </div>
      );
    }
  ...
  updateLeave = async (row, status) => {
    ...
    let res = await staffUpdateLeaves(params);
  ...
  render() {
    ...
    return <React.Fragment>{data && <BootstrapTable keyField="_id" data={data} columns={columns} bootstrap4 pagination={paginationFactory(options)} classes="table table-bordered table-hover table-sm" wrapperClasses="table-responsive" filter={filterFactory()} noDataIndication={"No data to display here"} />}</React.Fragment>;
  }
}
