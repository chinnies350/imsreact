...
import { staffLeaveInsert, staffListLeave } from 'services/leaveService';
...
to:new Date().toLocaleDateString(),
    ...
    schema = {
        ...
        to: Joi.string().required().label("Staff"),
    ...
    clientDatas = async (name, Input) => {
        ...
        switch (name) {
            ...
            case "department":
                ...
                await this.staffListLeave(`type=staff&client=${data.client}&type=department&entity=${data.entity}&branch=${data.branch}&department=${data.department}`)
    ...
    staffListLeave = async (params) => {
        console.log(params)
        const d = await staffListLeave(params);
        console.log(d)
        if (d && d.data && d.data.statusCode === 1)
            await this.setState({ staffList: d.data.data })
        else
    ...
    dateValue = async (date, field) => {
        ...
        // });
        await this.setState({from:new Date(from).toLocaleDateString(),to:new Date(to).toLocaleDateString()})
    }
    ...
    onSubmit = async () => {
        ...
        const data = this.formApi.getState().values
        const { client, entity, branch, department, batch, reason,to } = data

        let params = `client=${client}&entity=${entity}&branch=${branch}&department=${department}&batch=${batch}&uid=${this.state.uid}&name=${this.state.name}&status=request&reason=${reason}&fromDate=${data.date.from}&endDate=${data.date.to}&to=${to}`
        let res = await staffLeaveInsert(params)
        console.log(res)
        if (res.data.statusCode === 1) {
            this.props.props.history.push({
                pathname: `${subDirectory}/staffleave/leave`,
            })
    ...
    render() {
        const { clientIds, entityIds, branchIds, departmentIds, batchIds, isClient, isEntity, isBranch, isDepartment, isBatch,staffList,date } = this.state
                ...
                <Fragment>
                  ...
                  {/* Client as Group  and Entity as Institution*/}
                                            ...
                                            <Row>
                                                ...
                                                <Col sm={12} md={3}>
                                                    <CustomSelect field="to" label="Staff" name="to" getOptionValue={option => option.uid}
                                                        getOptionLabel={option => option.name} options={staffList}
                                                        validateOnBlur validate={e => this.validateProperty('to', e)} onChange={this.handleChange} />
                                                </Col>
                                            </Row>
                                        </section>
                                    }
        ...
        startDate={moment(this.state.from)} endDate={moment(this.state.to)}
                                                field="date" label="Date*" id="date" onChange={(data) => this.dateValue(data, "date")} validate={e => this.validateProperty('date', e)} />
                                            </Col>
                                        </Row>
                                        <Row>
                                            <Col sm="12" md="12">
