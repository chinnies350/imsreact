...
export default class RolesForm extends Component {
  ...
  schema = {
    ...
    type: Joi.string().required().label("Role Name"),
    userLevel: Joi.string().required().label("UserLevel"),
  };
  ...
  onSubmit = async () => {
    ...
    if (action === 'edit') {
      ...
      console.log(res,params,Roledata)
      if (res.data.statusCode === 1) {
        ToastService.Toast("Success", "default");
        this.props.props.history.goBack();
      } else {
    ...
    if (action === 'assign') {
      ...
      _.forEach(selected, async v => {
        ...
        console.log(obj)
        let response=await assignRoles(obj);
              ...
              <section>
                ...
                <Row>
                  ...
                  {isClient && <Col sm={12} md={3}>
                    <CustomSelect field="client" label="Group*" name="client" getOptionValue={option => option.code} getOptionLabel={option => option.name} options={clientIds} onChange={this.handleChange} validateOnBlur validate={e => this.validateProperty('client', e)} readOnly={readOnly} />
                  </Col>}
                  {isEntity &&
                    <Col sm={12} md={3}>
                      <CustomSelect field="entity" label="Institution*" name="entity" getOptionValue={option => option.code} getOptionLabel={option => option.name} options={entityIds} onChange={this.handleChange} validateOnBlur validate={e => this.validateProperty('entity', e)} readOnly={readOnly} />
                    </Col>}
                  {isBranch &&
                    <Col sm={12} md={3}>
                      <CustomSelect field="branch" label="Branch*" name="branch" getOptionValue={option => option.code} getOptionLabel={option => option.name} options={branchIds} onChange={this.handleChange} validateOnBlur validate={e => this.validateProperty('branch', e)} readOnly={readOnly} />
                    </Col>}
                  {action === 'edit' ?
                    (<Col sm={12} md={3}>
                      <Input field="type" label="Role Name*" name="type" onChange={this.handleChange} readOnly={readOnly} validateOnBlur validate={e => this.validateProperty('type', e)} readOnly/>
                    </Col>) :
                    (<Col sm={12} md={3}>
                      <Input field="type" label="Role Name*" name="type" onChange={this.handleChange} readOnly={readOnly} validateOnBlur validate={e => this.validateProperty('type', e)} />
                    </Col>)
                  }
                  {action === 'assign' && <Col sm={12} md={3}>
                    <CustomSelect field="userLevel" label="User Level*" name="userLevel" getOptionValue={option => option.name} getOptionLabel={option => option.name} options={userLevels} onChange={this.handleChange} validateOnBlur validate={e => this.validateProperty('userLevel', e)} />
                  </Col>}
                </Row>
              </section>
              {modulesDiv &&
