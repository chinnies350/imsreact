...
export default class MarkReportList extends Component {
  ...
  getColumnHeaders(type, prefixUrl = "", dynamicLabels = {}) {
    ...
    switch (type) {
      case 'gpa':
        ...
        break;
      case 'Scholastic':
        if (data && data['cce'] && data['cce']['Scholastic'] && data['cce']['Scholastic'][0]) {

          allKeys.push(_.keys(data['cce']['Scholastic'][0]))
        } else {
          allKeys.push('Scholastic')
        }
        excludeKeys = ["skill"];
        break;
      case 'co-scholastic':
        excludeKeys = ["skill"];
        if (data && data['cce'] && data['cce']['co-scholastic'] && data['cce']['co-scholastic'][0]) {

          allKeys.push(_.keys(data['cce']['co-scholastic'][0]))
        } else {
          allKeys.push('Co-Scholastic')
        }
  ...
  render() {
    ...
    const { form, formData,Students, studentUid } = this.props;
