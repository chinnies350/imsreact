  ...
  handleSubmit = async () => {
    ...
    if (type === 'assignment') {
      ...
      if (assignmentstatus.data.statusCode === 1) {
        ...
        await this.viewReport(this.state.assigndata)
  ...
  async viewReport(data) {
    ...
    for (let item of data) {
      if (item.assignmentReport) {
        ...
        assignmentView.push({ "studentId": studentId, "name": name, "topic": topic, "subject": subject, "marks": marks, "totalMarks": totalMarks, "remarks": remarks, "createdDate": createdDate })
        await this.setState({
    ...
    return (
      <Fragment>
        <div className="d-md-flex align-items-md-center justify-content-md-between maincontentheader">
          <h5 class="pg-title">{action} {type} Report</h5>
          <div>
