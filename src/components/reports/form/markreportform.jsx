...
export default class MarksReport extends Component {
  ...
  getStudentList = async () => {
    ...
    if (res.data.statusCode === 1) {
      ...
      for (var i = 0; i < test.length; i++) {
        ...
        await this.setState({Students:studentList})
        await this.setState({students:studentList})
  ...
  onSubmit = async () => {
    ...
    if (type === 'mark') {
      const { client, entity, branch, department, batch, term, uid, userType,Students, studentId } = formdata
      console.log(formdata)
      let params = '';
      params = `uid=${this.state.studentId}&batch=${batch}&department=${department}&client=${client}&entity=${entity}&branch=${branch}&term=${term}`

      // params = `uid=${this.state.students}&batch=${batch}&department=${department}&client=${client}&entity=${entity}&branch=${branch}&term=${term}`
      ...
      console.log(markReport,params, formdata)
  ...
  renderReportsForm(data, formData, studentId) {
    const { rightsData } = this.props;
    const { markSystem ,Students} = this.state;
    return <MarkReportList form={markSystem} data={data} formData={formData} rightsData={rightsData} studentUid={studentId} Students={Students} />
  }

  autoSujestValue = async (value) => {
    await this.setState({
      studentId: value
    })
    console.log(value)
  ...
  render() {
    const { clientIds, Students,entityIds, branchIds, departmentIds, batchIds, studentList, formdata, isTableLoading, isClient, isEntity, isBranch, isDepartment, tableData, isBatch, studentId, data: { userType } } = this.state;
    ...
    return (
      <Fragment >
          ...
          {({ formApi, formState }) => (
            <div>
              <div className="d-md-flex align-items-md-center justify-content-md-between maincontentheader">
                <h5 class="pg-title">Mark Report</h5>
                <div>
                ...
                <Row>
                  ...
                  <Col sm={6} md={3}>
                   ...
                   < Col sm={6} md={3}>
                     ...
                     {/* <CustomSelect field="students" label="Students*" name="students" 
                   
                     getOptionValue={option => option.code} 
                     getOptionLabel={option => option.name} validateOnBlur validate={e => this.validateProperty('students', e)}
                     options={this.state.Students} onChange={this.handleChange} /> */}
        ...
        {
          !isTableLoading && formdata && studentId &&
          this.renderReportsForm(tableData, formdata, studentId)
        }
