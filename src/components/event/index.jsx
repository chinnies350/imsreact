...
export default class EventRoot extends Component {
  ...
  rightsData = async (session) => {
    let res = await rightsData("events", session);
    let excludeModules = [];
  ...
  addeventNavigation(eventformType) { // navidate to Add Attendees Page        
    return <NavLink onClick={this.redirectTo} className="btn btn-primary btn-sm" to={`${subDirectory}/event/add/${eventformType} `}>+ {eventformType} </NavLink>
  }
  ...
  render() {
    ...
    if (_form === 'addAttendees') _form = "Event Registration"
    if (_form === 'gallery') _form = "Gallery"
          ...
          {!isPageLoading && !isLoading &&
            <Fragment>
              ...
              <Breadcrumb>
                ...
                <BreadcrumbItem><NavLink to={`${subDirectory}/event/addAttendees`}>event</NavLink></BreadcrumbItem>
              ...
              <Container fluid>
                ...
                <div className="d-md-flex align-items-md-center justify-content-md-between maincontentheader">
                  ...
                  {eventformType === 'addAttendees' &&
                    <div onClick={this.bulkModalToggle} >
