...
export default class EventAttendeesForm extends Component {
  ...
  schema = { //Validate all the Feilds present in this Modules
    ...
    fee: Joi.number().min(0).required().label('Fees'),
  ...
  resetForm = () => {
    this.formApi.reset()
    let path = `${subDirectory}/event/addAttendees` //Redirect the page after updated the datas
  ...
  onSubmit = async () => { //Store Datas to the APIs
    ...
    let updateParams = addParams + `&departmentId=${data.departmentId}&batchId=${data.batchId}&student=${this.state.studentName}`
    let addEvents = {
    ...
    if (actiontype === 'add')
      ...
      console.log(response,updateParams,updateEventData)
    }
  ...
  render() {
    ...
    const { clientIds, entityIds, branchIds, departmentIds, batchIds, studentList,
      isClient, isEntity, isBranch, isDepartment, isBatch,Students,studentName
    } = this.state
